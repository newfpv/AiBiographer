...
- После обработки бот пытается удалять пользовательские чувствительные сообщения (API-ключ, загруженный экспорт, ручной target/model input).
...
Открытие админ-панели (только `ADMIN_ID`):
```bash
/admin
```
